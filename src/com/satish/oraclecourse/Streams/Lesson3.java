...
public class Lesson3 {
  ...
  static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
   ...
   ( parallel ? wordList.parallelStream():wordList.stream())
            .map(w -> {return (int[])wordList.stream()
    		    .mapToInt( w1->{ return Levenshtein.lev(w, w1);})
    		    .toArray();})
  ...
  static List<String> processWords(List<String> wordList, boolean parallel) {
    // YOUR CODE HERE
   // List<String> newList = (parallel? wordList.parallelStream() : wordList.stream()).sorted().collect(Collectors.toList());
  //  List<String> newList = (parallel? wordList.parallelStream() : wordList.stream()).distinct().collect(Collectors.toList());
	   List<String> newList = (parallel? wordList.parallelStream() : wordList.stream()).map(String::toLowerCase).collect(Collectors.toList());
    return newList;
  }
